export default function PermiumCalls({ title }) {
  return (
    <div>
      <h3>{title}</h3>Host video calls with hundreds of people, get moderator
      controls, and more.{" "}
      <button>
        <a href="https://account.brave.com/plans/?intent=checkout&product=talk">
          Start free trial
        </a>
      </button>{" "}
      Trial brings 30 days of free access to Brave Talk Premium. After 30 days,
      the credit card you enter will be charged $7.00 US per month. Cancel any
      time. Already have Premium? Log in.
    </div>
  );
}
